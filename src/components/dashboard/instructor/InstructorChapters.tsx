...
import { 
  ArrowLeft, Plus, Edit, Trash2, GripVertical, FolderOpen, BookOpen, Video 
} from 'lucide-react';
import { toast } from 'sonner';
import { InstructorLessons } from './InstructorLessons';

interface InstructorChaptersProps {
  courseId: string;
  courseTitle: string;
  onBack: () => void;
}

export const InstructorChapters = ({ courseId, courseTitle, onBack }: InstructorChaptersProps) => {
  ...
  // Get lesson counts per chapter and unassigned lessons
  const { data: lessonCountsData = { counts: {}, unassignedCount: 0 } } = useQuery({
    queryKey: ['chapter-lesson-counts', courseId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('lessons')
        .select('chapter_id')
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      resetForm();
      // Navigate to lessons page for the new chapter
